...
// AudioItem = { id: string, name: string, uri: string, duration: number, size: string }

export default function AudioScreen() {
  const [recording, setRecording] = useState(null);
  const [audioList, setAudioList] = useState([]);
  const [filteredAudioList, setFilteredAudioList] = useState([]);
  ...
  const loadAudios = async () => {
    ...
    if (stored) {
      const parsed = JSON.parse(stored);
      setAudioList(parsed);
      setFilteredAudioList(parsed);
    }
  };

  const saveAudio = async (newAudio) => {
    const updated = [...audioList, newAudio];
    setAudioList(updated);
    setFilteredAudioList(updated);
    await AsyncStorage.setItem('audioList', JSON.stringify(updated));
